      ...
      <div className="max-w-300 mx-auto px-6 md:px-10">
        ...
        <SectionReveal delay={100}>
          <h2 className="font-display text-[clamp(1.8rem,4vw,3rem)] text-(--ink) font-semibold mt-4 mb-16">
            The craft, the tools,{" "}
            <span className="text-(--accent)">the stack.</span>
          </h2>
        </SectionReveal>
        ...
        <div className="flex flex-col gap-6">
          {categories.map((cat, i) => (
            <SectionReveal key={cat.key} delay={100 + i * 100}>
              <div
                className="group grid sm:grid-cols-[160px_1fr] gap-4 sm:gap-8 items-start border border-(--border) p-6 md:p-8 relative overflow-hidden"
                style={{ boxShadow: "var(--shadow-sm)" }}
              >
                {/* Hover bg fill */}
                <div className="absolute inset-0 bg-(--accent) opacity-0 group-hover:opacity-[0.03] transition-opacity duration-300 pointer-events-none" />
                {/* Top accent line */}
                <div className="absolute top-0 left-0 right-0 h-px bg-(--border) group-hover:bg-(--accent-pop) transition-colors duration-300" />
                <div className="pt-1 relative z-10">
                  <span className="text-[11px] font-mono text-(--ink-muted) uppercase tracking-widest">
                    {cat.label}
                  </span>
                  <div className="w-8 h-px bg-(--accent-pop) mt-2" />
                </div>
